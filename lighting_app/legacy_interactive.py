...
# GPU acceleration (NVIDIA CUDA via CuPy, or Intel Arc/AMD/Apple via Taichi+Vulkan)
# — lazy init, no GPU driver loaded at import time
try:
    from gpu_raytrace import gpu_process_led_wall_batch, gpu_process_room_batch
    from gpu_raytrace import GPU_AVAILABLE as _GPU_OK, GPU_BACKEND as _GPU_BACKEND, _ensure_gpu_init
...
# Mesh lighting legend (gradient from dark blue to white)
    ...
    def compute_wall_intensity(
        ...
        if _HAS_GPU_MODULE:
            _ensure_gpu_init()
            from gpu_raytrace import GPU_AVAILABLE, GPU_BACKEND
        ...
        if use_gpu:
            ...
            if stl_mesh_data is not None:
                ...
                backend_label = "NVIDIA CUDA" if GPU_BACKEND == 'cuda' else "Vulkan (Intel Arc/AMD/Apple)"
                print(f"[GPU] Using {backend_label} acceleration for {len(active_leds)} LEDs...")
    ...
    def compute_room_intensity(
        ...
        if use_gpu:
            ...
            if stl_mesh_data is not None:
                ...
                # === PURE GPU PATH (no mesh) ===
                backend_label = "NVIDIA CUDA" if GPU_BACKEND == 'cuda' else "Vulkan (Intel Arc/AMD/Apple)"
                print(f"[GPU] Using {backend_label} acceleration for room mode ({len(active_leds)} LEDs)...")
                gpu_grids, gpu_hits, total_rays = gpu_process_room_batch(leds_data, gpu_params)
                for wall_name in grids.keys():
                    grids[wall_name] = gpu_grids[wall_name]
                    ray_hits[wall_name] = gpu_hits.get(wall_name, 0)
